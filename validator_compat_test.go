...
func BenchmarkArgusFieldSuccess(b *testing.B) {
	v := New()
	for i := 0; i < b.N; i++ {
		if err := v.Var("argus@example.com", "required,email"); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkArgusFieldFailure(b *testing.B) {
	v := New()
	for i := 0; i < b.N; i++ {
		_ = v.Var("bad-email", "required,email")
	}
}

func BenchmarkArgusStructSimpleSuccess(b *testing.B) {
	v := New()
...
func BenchmarkArgusOneOf(b *testing.B) {
	v := New()
	for i := 0; i < b.N; i++ {
		if err := v.Var("green", "oneof=red green blue"); err != nil {
			b.Fatal(err)
		}
	}
}
